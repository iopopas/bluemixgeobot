...
var PayloadPanel = (function() {
  ...
  function payloadUpdateSetup() {
    ...
    Api.setRequestPayload = function(newPayloadStr) {
      ...
      //Ionel: disable the payload response display
    ...
    Api.setResponsePayload = function(newPayload) {
      ...
      responseJson = JSON.parse(newPayload);
      if(responseJson.intents) {
        if(responseJson.intents.length >0){
          if (responseJson.intents[0].intent == "zoom_to_ca"){
            postMessageToMap("zoomToCA");
          }
          if (responseJson.intents[0].intent == "zoom_to_co"){
            postMessageToMap("zoomToCO");
          }
          if (responseJson.intents[0].intent == "fires_2016"){
            postMessageToMap("geobot_fires_2016");
          }
          if (responseJson.intents[0].intent == "fires_2017"){
            postMessageToMap("geobot_fires_2017");
          }
          if (responseJson.intents[0].intent == "active_fires"){
            postMessageToMap("geobot_active_fires");
          }
        }
      }
  ...
  function postMessageToMap(message){
